...
public class MainActivity extends Activity {
	...
	/**
	 * 非同期処理を実行するためのネストクラス。 Activity などのライフサイクルに合わせた管理は自分でする必要があるが、
	...
	private class MyAsyncTask extends AsyncTask<Void, Void, Void> {
		...
		protected void onProgressUpdate(Void... values) {
			...
			Toast.makeText(MainActivity.this, "onProgressUpdate",
					Toast.LENGTH_SHORT).show();
		}

		/**
		 * 非同期処理の本体で、UI スレッドではない別のスレッドで処理する内容。 引数は非同期処理内容に渡すためのパラメータの配列。
		 */
		@Override
		protected Void doInBackground(Void... params) {
			...
			try {
				...
				Log.e(MyAsyncTask.class.getSimpleName(),
		...
		/**
		 * 非同期処理の実行後に、UI スレッドで実行する処理。 引数は {@link AsyncTask#execute(Object...)}
		 * の返り値。
		 */
		@Override
